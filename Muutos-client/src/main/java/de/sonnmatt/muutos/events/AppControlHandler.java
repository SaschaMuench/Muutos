...
public class AppControlHandler implements IAppControlHandler, ValueChangeHandler<String> {
	...
	private void doAppStart() {
		...
		AppService.Util.getInstance().getData(AppLocalData.getSessionID(), new AsyncCallback<DataDTO>() {
			...
			public void onSuccess(DataDTO result) {
				...
				TranslationsDTO transDTO = (TranslationsDTO) result.get(dataTypes.Translation);
	...
	private void doLogin() {
		...
		try {
			LoginService.Util.getInstance().getText(language, TranslationSections.Login, new AsyncCallback<TranslationsDTO>() {
	...
	private void doLogoff(boolean timeout) {
		...
		TranslationSections action = timeout == true ? TranslationSections.Timeout : TranslationSections.Logoff;
		try {
			LoginService.Util.getInstance().getText(language, action, new AsyncCallback<TranslationsDTO>() {
